...
int main(int argc, char** argv)
{
	...
	for(int i=0;i<imgs.size();++i)
	{
		...
		for(int j=i+1;j<imgs.size();++j)
		{
			...
			for(int k=j+1;k<imgs.size();++k)
			{
                ...
                // this check is counting the number of common points between images
                ...
                // (EXTRA NOTE) when this point of the code is reached the images i,j,k have some points in common
                // (EXTRA NOTE) sample is the number passed as argument, the sample numbers limited in sample2 if the "ok" common points
                //  are noth enough
    			...
    			for(int a=0;a<samples2;++a)
    			{
            		...
            		bool found = 0;
                    std::cout << "\nimage-ids " << imgs[i].id << " " << imgs[j].id << " "<< imgs[k].id;
                    std::cout << "\n3d-all";
                    // Print all values in the ok vector separated by a dash
                    for (int i : ok) {
                        std::cout << " " << i;
                    }
                    std::cout << "\n3d-selected";
            		for(int b=0;b<4;b++)
            		{
                        //(EXTRA NOTE) U1, U2, U3 contains the non normalized "problem formulation", it is normalized in the next loop
            			points1[b] = (U1[ok[smpl[b]]]);
            			points2[b] = (U2[ok[smpl[b]]]);
            			points3[b] = (U3[ok[smpl[b]]]);

                        //(EXTRA NOTE) these are the identifier of the selected points, they are unique among all images
                        std::cout << " " <<ok[smpl[b]];
            		}
                    std::cout << "\nproblem ";
    ...
    std::string outputFileName = "./file.txt";
